...
class UserModel extends BaseModel {
    ...
    public function getAllPaginated(int $page, string $role = ''): array {
        ...
        $offset = ($page - 1) * $limit;
        if ($role) {
            $result = $this->execute('SELECT * FROM users WHERE role=? LIMIT ? OFFSET ?', 'sii', [$role, $limit, $offset]);
        } else {
            $result = $this->execute('SELECT * FROM users LIMIT ? OFFSET ?', 'ii', [$limit, $offset]);
        }
    ...
    public function toggleActive(int $id): bool {
        $result = $this->execute('UPDATE users SET is_active = 1 - is_active WHERE id=?', 'i', [$id]);
        return $result === true;
    }
}
